...
export default function Dashboard() {
      ...
      if (uData.flatId) {
        ...
        if (flatSnap.exists()) {
          ...
          if (fData.members && fData.members.length > 0) {
            ...
            // ✅ FIX 1: uid ko explicitly include karo — snap.data() mein uid hoti nahi hai
            setFlatMembers(
              memberSnaps
            ...
            setFlatTransactions(txnsSnap.docs.map(d => ({ id: d.id, ...d.data() })));
          }
        }
      } else {
        setFlatData(null);
    ...
    if (flatData.createdBy === user.uid && flatData.members.length > 1) {
      ...
      for (const memberId of flatData.members) {
        if (memberId !== user.uid) {
          ...
          if (mSnap.exists()) others.push({ uid: mSnap.id, ...mSnap.data() });
        }
      }
  ...
  const handleViewPastFlat = async (pf: any) => {
    ...
    try {
      ...
      const txnsSnap = await getDocs(q);
      setPastTransactions(txnsSnap.docs.map(d => ({ id: d.id, ...d.data() })));
    } catch (e) {
      console.error(e);
      setPastTransactions([]);
    } finally {
  ...
  const openSettleModal = async (debt: any) => {
    ...
    try {
      ...
      if (uSnap.exists()) {
        setSettleData({ toUser: { uid: uSnap.id, ...uSnap.data() }, recommendedAmount: debt.amount });
  ...
  // ✅ FIX 2: Screenshot optional — approval still required
  const handleSubmitPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!settleAmount || Number(settleAmount) <= 0) {
      return setErrorMsg("Valid amount required.");
    }
    setIsSubmitting(true);
    setErrorMsg("");
    try {
      // Proof is optional — upload only if provided
      let proofBase64 = "";
      if (proofFile) {
      ...
      await setDoc(doc(db, "transactions", txnId), {
        ...
        proofUrl: proofBase64, // empty string if no proof uploaded
        date: new Date().toISOString(),
        type: "settlement",
  ...
  // --- CALCULATE BALANCES (Greedy Algorithm base) ---
  const calculateBalances = () => {
    const balances: { [uid: string]: number } = {};
    // Initialize all members at 0
    flatMembers.forEach(m => { if (m.uid) balances[m.uid] = 0; });
    ...
    flatTransactions.forEach(txn => {
      if (txn.type === "expense") {
        ...
        if (txn.paidBy && balances[txn.paidBy] !== undefined) {
        ...
        // from (debtor) paid, so their debt reduces (balance goes up)
        if (txn.from && balances[txn.from] !== undefined) {
          balances[txn.from] += txn.amount;
        }
        // to (creditor) received, so their credit reduces (balance goes down)
        if (txn.to && balances[txn.to] !== undefined) {
  ...
  const calculateDetailedDebts = (balances: { [uid: string]: number }) => {
    // Debtors: negative balance (they owe money)
    let debtors: { uid: string; amount: number }[] = [];
    // Creditors: positive balance (they are owed money)
    let creditors: { uid: string; amount: number }[] = [];
    ...
    // Sort descending for better greedy matching
    ...
    while (d < debtors.length && c < creditors.length) {
      ...
      debts.push({ from: debtor.uid, to: creditor.uid, amount });
      ...
      <div className="flex justify-between items-center p-3 bg-white rounded-xl border border-gray-100 shadow-sm group">
        <div className="flex-1">
          <p className="font-bold text-sm text-[#1D1D1F]">
            ...
            {txn.type === "settlement" && txn.status === "pending" && (
              <span className="ml-2 text-[10px] bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">Pending</span>
            )}
          </p>
          <p className="text-xs text-gray-400 mt-0.5">
            {new Date(txn.date).toLocaleDateString()} • {txn.type === "settlement"
              ? `From ${txn.from === user.uid ? "You" : flatMembers.find(m => m.uid === txn.from)?.name?.split(" ")[0] || "Unknown"}`
              : `Paid by ${txn.paidBy === user.uid ? "You" : flatMembers.find(m => m.uid === txn.paidBy)?.name?.split(" ")[0] || "Unknown"}`
            }
          </p>
        </div>
                  ...
                  detailedDebts.map((debt, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        {/* ✅ Pay button shows when user is debtor and flat is not locked */}
                        {debt.from === user.uid && !isLocked && (
                          <button
                            onClick={() => openSettleModal(debt)}
                            className="bg-black text-white text-xs px-3 py-1.5 rounded-lg font-bold hover:bg-gray-800 active:scale-95 transition-all"
                          >
                            Pay
                          </button>
                        )}
        ...
        {modalType && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className={`bg-white w-full ${modalType === 'ledger' ? 'max-w-2xl' : 'max-w-md'} rounded-[24px] shadow-2xl relative z-10 p-6 sm:p-8 overflow-hidden max-h-[90vh] flex flex-col`}>
              ...
              {/* ✅ INITIATE PAYMENT MODAL — Screenshot Optional, Approval Still Required */}
                ...
                <form onSubmit={handleSubmitPayment} className="overflow-y-auto pr-2">
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    <div>
                      ...
                      <label className="block text-sm font-semibold text-[#86868B] mb-1 ml-1">
                        Upload Payment Screenshot <span className="text-gray-400 font-normal">(Optional)</span>
                      </label>
                      <p className="text-xs text-gray-400 mb-2 ml-1">Approval from receiver is still required regardless.</p>
                      <label className="w-full flex items-center justify-center gap-2 bg-[#F5F5F7] hover:bg-gray-200 text-[#1D1D1F] px-4 py-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors border border-dashed border-gray-300">
                        <Upload className="w-4 h-4" /> {proofFile ? proofFile.name : "Select Screenshot (Optional)"}
                  ...
                  {/* ✅ Removed !proofFile from disabled condition */}
                  <button type="submit" disabled={isSubmitting} className="w-full bg-[#34C759] hover:bg-[#2EAF4E] disabled:opacity-50 text-white py-3.5 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-sm">
                      ...
                      {pendingApprovals.map(txn => (
                        <div key={txn.transactionId} className="bg-gray-50 border border-gray-200 p-4 rounded-2xl">
                          ...
                          {txn.proofUrl ? (
                            ...
                            <p className="text-xs text-gray-400 italic mb-3">No screenshot provided.</p>
                          )}
